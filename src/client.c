        ...
        if (send_message.length > 1) {
            ...
            if ((len = recv(client_socket, &(recv_message), sizeof(message), 0)) > 0) {
                if ((recv_message.status == OK_WAIT_FOR_RESPONSE || recv_message.status == OK_DONE) &&
                    ...
                    int num_buffers = floor_func(num_bytes, DEFAULT_STDIN_BUFFER_SIZE);
                    for (int i = 0; i < num_buffers; i++) {
                        char payload[DEFAULT_STDIN_BUFFER_SIZE + 1];
                        int buffer_len = DEFAULT_STDIN_BUFFER_SIZE;
                        if (i == num_buffers - 1) {
                            buffer_len = num_bytes - i * DEFAULT_STDIN_BUFFER_SIZE;
                        }
                        // printf("%d %d %d\n", i, buffer_len, num_bytes);
                        // Receive the payload and print it out
                        if ((len = recv(client_socket, payload, buffer_len, 0)) > 0) {
                            payload[buffer_len] = '\0';
                            printf("%s", payload);
                        }
                    }
                    printf("\n");
